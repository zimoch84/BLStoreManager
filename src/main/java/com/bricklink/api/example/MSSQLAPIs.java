 ...
 * @author Bartosz
 */
public class MSSQLAPIs {
    MSSQLConnection connectiion;
    BLAPIs blAPI;
    boolean debug = false;
    
public MSSQLAPIs() {
    this.connectiion =   new MSSQLConnection();
    this.blAPI = new BLAPIs();
}

    public MSSQLAPIs(boolean debug) {
        this.debug = debug;
        this.connectiion =   new MSSQLConnection();
...
void refreshInventoryDatabase(){
    ...
    String updatequery = "UPDATE BARTEK.INVENTORY set isvalid = 'N' , dto=sysdate where isvalid = 'Y'"   ;
        try {
            PreparedStatement ps = connectiion.getConnection().prepareStatement(updatequery);
        ...
        for(int i=0; i<json.getJSONArray("data").length(); i++){
            ...
            try {
                ...
                PreparedStatement ps = connectiion.getConnection().prepareStatement(query);
                ...
                Logger.getLogger(MSSQLAPIs.class.getName()).log(Level.SEVERE, null, ex);
            }
...
ResultSet rs=connectiion.sendRequest(dbQuerry);
try {
    while (rs.next()) {
        ...
        int bl_unit_quantity  = temp.getInt("unit_quantity");
            ...
            if(bl_avg_price > 0 )
            {
                ...
                CallableStatement cs = connectiion.getConnection().prepareCall("{call INSERT_PRICE(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");//14xin 1xout
                ...
                for(int i=0; i<price_detail.length(); i++){
                 ...
                 if(custom_guide_type.equals("sold") ){
                    ...
                    cs = connectiion.getConnection().prepareCall("{call INSERT_PRICE_BY_SHOP(?,?,?,?,?,?,?,?,?,?,?,?)}");//7in 
                    cs.setString(1, invPartID);
...
ResultSet rs=connectiion.sendRequest(dbQuerryFromItems);
try {
    while (rs.next()) {
    count_price++;
    String invPartID = rs.getString("PART_ID");
...
try {
    ...
    if(invType.equals("SET")  )
    {
        ...
        CallableStatement cs = connectiion.getConnection().
